...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user is not None and bcrypt.check_password_hash(user.password, user.otp_secret + form.password.data) and user.verify_totop(form.token.data):
            login_user(user)
...
def account():
    ...
    if form.validate_on_submit():
        ...
        current_user.password = bcrypt.generate_password_hash(current_user.otp_secret + form.password.data).decode('utf-8')
        db.session.commit()
        return redirect(url_for('users.account'))
